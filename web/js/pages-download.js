...
async function loadDownloadQueue() {
    ...
    try {
        ...
        if (result.success) {
            ...
            // 如果已完成，自动更新队列状态
            await checkAndUpdateCompletedQueueItems();
        ...
        // 检测是否有正在进行的批量下载
        await checkAndShowBatchProgress();
    } catch (e) {
...
// 检查并更新已完成的队列项目
async function checkAndUpdateCompletedQueueItems() {
...
async function checkAndShowBatchProgress() {
    ...
    // 如果最近检查过（1秒内），使用缓存的结果
    if (lastBatchProgressCheck && Date.now() - lastBatchProgressCheck.timestamp < 1000) {
        if (lastBatchProgressCheck.data) {
            updateQueueBatchProgressWithData(lastBatchProgressCheck.data);
            // 如果有正在运行的任务，启动轮询
            if (lastBatchProgressCheck.data.running > 0) {
                startQueueBatchProgressMonitoring();
            }
        }
        return;
    }

    batchProgressCheckInProgress = true;
    try {
        const data = await ApiClient.getBatchProgress();
        lastBatchProgressCheck = { data, timestamp: Date.now() };
        if (data.success && data.total > 0) {
            // 检查任务是否已全部完成（done + failed = total 且 running = 0）
            const allCompleted = (data.done + data.failed === data.total) && data.running === 0;

            if (allCompleted) {
                // 任务已全部完成，自动清除任务（避免刷新页面时显示旧任务）
                console.log('批量下载任务已全部完成，自动清除任务');
                try {
                    await ApiClient.clearBatchDownload();
                    // 隐藏进度卡片
                    const card = document.getElementById('queueBatchProgressCard');
                    if (card) {
                        card.style.display = 'none';
                    }
                    if (window.queueBatchProgressInterval) {
                        clearInterval(window.queueBatchProgressInterval);
                        window.queueBatchProgressInterval = null;
                    }
                } catch (e) {
                    console.error('Failed to clear batch tasks:', e);
                    // 即使清除失败，也显示进度卡片
                    const card = document.getElementById('queueBatchProgressCard');
                    if (card) {
                        card.style.display = 'block';
                    }
                    updateQueueBatchProgressWithData(data);
                }
            } else {
                // 有批量下载任务，显示进度卡片
                const card = document.getElementById('queueBatchProgressCard');
        ...
        batchProgressCheckInProgress = false;
    }
}

// 开始监控批量下载进度（下载队列页面）
function startQueueBatchProgressMonitoring() {
    if (window.queueBatchProgressInterval) {
        clearInterval(window.queueBatchProgressInterval);
    }
    updateQueueBatchProgress();
    window.queueBatchProgressInterval = setInterval(updateQueueBatchProgress, 2000);
}

// 更新批量下载进度（下载队列页面）- 使用已有数据
async function updateQueueBatchProgressWithData(data) {
    if (!data || !data.success) return;

    // 更新统计数据
    const el = (id) => document.getElementById(id);
    if (el('queueBatchTotal')) el('queueBatchTotal').textContent = data.total || 0;
    if (el('queueBatchDone')) el('queueBatchDone').textContent = data.done || 0;
    if (el('queueBatchFailed')) el('queueBatchFailed').textContent = data.failed || 0;
    if (el('queueBatchRunning')) el('queueBatchRunning').textContent = data.running || 0;

    // 更新进度条
    // 计算总进度：已完成 + 失败 + 所有正在下载任务的进度
    let percentage = 0;
    if (data.total > 0) {
        const completedProgress = data.done + data.failed;
        // 计算所有正在下载任务的进度总和
        let downloadingProgress = 0;
        if (data.tasks && Array.isArray(data.tasks)) {
            const downloadingTasks = data.tasks.filter(t => t.status === 'downloading');
            downloadingProgress = downloadingTasks.reduce((sum, task) => {
                return sum + (task.progress || 0) / 100;
            }, 0);
        }
        percentage = Math.round((completedProgress + downloadingProgress) / data.total * 100);
    }
    const progressBar = el('queueBatchProgressBar');
    const progressText = el('queueBatchProgressText');
    if (progressBar) {
    ...
    if (downloadingTasksContainer) {
        if (data.tasks && Array.isArray(data.tasks)) {
            ...
            if (downloadingTasks.length > 0) {
                    ...
                    html += `
                        <div class="queue-item downloading" style="margin-bottom: 12px;">
                            <div class="queue-item-info" style="flex: 1;">
                                <div style="font-size: 12px; color: var(--text-muted); margin-bottom: 4px;">正在下载</div>
                ...
                downloadingTasksContainer.innerHTML = html;
            } else {
                downloadingTasksContainer.innerHTML = '';
            }
        } else {
            downloadingTasksContainer.innerHTML = '';
        }
    ...
    // 隐藏单个当前任务显示（如果存在）
    const currentTaskEl = el('queueBatchCurrentTask');
    if (currentTaskEl) {
        currentTaskEl.style.display = 'none';
    }

    // 检查下载状态：完成、已取消或已清除
    // 检查是否有任务被取消（error 包含"取消"或 status 为 failed 且 error 为"下载已取消"）
    const hasCancelledTask = data.tasks && Array.isArray(data.tasks) &&
        data.tasks.some(task =>
            (task.error && task.error.includes('取消')) ||
            (task.status === 'failed' && task.error === '下载已取消')
        );

    const isCompleted = data.done + data.failed === data.total && data.total > 0 && !hasCancelledTask;
    const isCancelled = hasCancelledTask || (data.running === 0 && data.total > 0 && !isCompleted);
    const isCleared = data.total === 0; // 任务已清除

    // 更新状态文本
    const statusText = el('queueBatchStatusText');
    if (statusText) {
        if (isCleared) {
            ...
            statusText.style.color = 'var(--success-color, #52c41a)';
        } else if (isCancelled) {
            statusText.textContent = '已取消';
            statusText.style.color = 'var(--warning-color, #faad14)';
        } else {
    ...
    // 如果任务已清除，隐藏进度卡片
    if (isCleared) {
        const card = el('queueBatchProgressCard');
        if (card) card.style.display = 'none';
        if (window.queueBatchProgressInterval) {
            clearInterval(window.queueBatchProgressInterval);
            window.queueBatchProgressInterval = null;
        }
        updateQueueBatchButtons(false, 0, false);
        return;
    }

    // 如果下载完成或已取消，更新按钮状态
    if (isCompleted || isCancelled) {
        updateQueueBatchButtons(isCompleted, data.failed || 0, isCancelled);

        if (isCompleted) {
            // 下载完成，停止轮询（但不隐藏卡片，用户可以查看结果或继续操作）
            if (window.queueBatchProgressInterval) {
                clearInterval(window.queueBatchProgressInterval);
                window.queueBatchProgressInterval = null;
            }
        } else if (isCancelled) {
            // 下载已取消，停止轮询（保持卡片显示，用户可以继续下载）
            if (window.queueBatchProgressInterval) {
                clearInterval(window.queueBatchProgressInterval);
                window.queueBatchProgressInterval = null;
            }
        ...
        // 下载进行中，确保取消按钮可见
        updateQueueBatchButtons(false, data.failed || 0, false);
    }
}

// 更新批量下载进度（下载队列页面）
async function updateQueueBatchProgress() {
    try {
        const data = await ApiClient.getBatchProgress();
        if (data.success) {
            // 更新统计数据
            ...
            // 更新任务列表（如果需要显示任务列表，取消下面的注释并添加对应的HTML容器）
            // if (data.tasks && Array.isArray(data.tasks)) {
            //     renderQueueBatchTasksList(data.tasks);
            // }
        ...
        console.error('Failed to update queue batch progress:', e);
    }
}
        ...
        try {
            ...
            if (progress.success && progress.total > 0) {
                ...
                if (progress.done > lastDone || (progress.running === 0 && progress.done > 0)) {
                    ...
                    // Note: Backend automatically completes download, no need to call API
                    // Removed ApiClient.completeDownload() to prevent duplicate records
                    item.status = 'completed';
                    item.downloadedSize = item.totalSize;
                    renderQueueList();
                    updateQueueStats();
...
async function resumeAllDownloads() {
    ...
    if (pausedItems.length > 0) {
        try {
            ...
            showMessage('恢复暂停任务失败: ' + e.message, 'error');
        }
    }
        ...
        if (readyItems.length > 0) {
            try {
                ...
                if (result.success) {
                    ...
                    showMessage(`已提交 ${readyItems.length} 个视频开始下载`, 'success');
                    // 监控下载进度
                    if (readyItems.length > 0) {
...
// 检测并显示批量下载进度（批量下载页面）
async function checkAndShowBatchProgressForBatchPage() {
    // 如果正在检查中，跳过
    if (batchProgressCheckInProgress) {
        return;
    }
    ...
    if (lastBatchProgressCheck && Date.now() - lastBatchProgressCheck.timestamp < 1000) {
        if (lastBatchProgressCheck.data) {
            updateBatchProgressWithData(lastBatchProgressCheck.data);
            // 如果有正在运行的任务，启动轮询
            if (lastBatchProgressCheck.data.running > 0) {
                startBatchProgressMonitoring();
            }
    ...
    try {
        ...
        if (data.success && data.total > 0) {
            ...
            const batchLogCard = document.getElementById('batchLogCard');
            if (batchProgressCard) batchProgressCard.style.display = 'block';
            if (batchLogCard) batchLogCard.style.display = 'block';

            // 直接使用获取到的数据更新UI，避免重复请求
            updateBatchProgressWithData(data);
            ...
            if (batchLogCard) batchLogCard.style.display = 'none';
            if (batchProgressInterval) {
                clearInterval(batchProgressInterval);
                batchProgressInterval = null;
            }
        ...
        console.error('Failed to check batch progress for batch page:', e);
    } finally {
        batchProgressCheckInProgress = false;
    }
}

// 更新批量下载进度（批量下载页面）- 使用已有数据
async function updateBatchProgressWithData(data) {
    if (!data || !data.success) return;
    ...
    // 显示所有正在下载的任务
    const downloadingTasksContainer = document.getElementById('batchDownloadingTasks');
    if (downloadingTasksContainer) {
        if (data.tasks && Array.isArray(data.tasks)) {
            const downloadingTasks = data.tasks.filter(t => t.status === 'downloading');
            if (downloadingTasks.length > 0) {
    ...
    if (batchStatusText) {
        if (isCompleted) {
            batchStatusText.textContent = '已完成';
            batchStatusText.style.color = 'var(--success-color, #52c41a)';
        } else if (isCancelled) {
            batchStatusText.textContent = '已取消';
            batchStatusText.style.color = 'var(--warning-color, #faad14)';
        } else {
            batchStatusText.textContent = '进行中...';
            batchStatusText.style.color = 'var(--text-muted, #888)';
        }
    }
    ...
    if (isCompleted) {
        ...
        updateBatchButtons(true, data.failed, false);
    } else if (isCancelled) {
        clearInterval(batchProgressInterval);
        batchProgressInterval = null;
        updateBatchButtons(false, data.failed || 0, true);
    } else {
        updateBatchButtons(false, data.failed || 0, false);
    }
}
...
async function updateBatchProgress() {
    try {
        ...
        if (data.success) {
            ...
            if (data.tasks && Array.isArray(data.tasks)) {
                updateBatchLog(data.tasks);
                // 渲染任务列表（如果需要显示任务列表，取消下面的注释并添加对应的HTML容器）
                // renderBatchTasksList(data.tasks);
            }
...
// 渲染任务列表（批量下载页面）
function renderBatchTasksList(tasks) {
    const container = document.getElementById('batchTasksList');
    if (!container) return;

    if (!tasks || tasks.length === 0) {
        container.innerHTML = '<div style="color: var(--text-muted); padding: 16px; text-align: center;">暂无任务</div>';
        return;
    }
    ...
    tasks.forEach(task => {
        const statusClass = task.status === 'done' ? 'success' :
            task.status === 'failed' ? 'error' :
                task.status === 'downloading' ? 'info' : 'muted';
        ...
        html += `
            <div class="batch-task-item" style="padding: 12px; border-bottom: 1px solid var(--border-color); display: flex; align-items: center; gap: 12px;">
                <div style="flex: 1; min-width: 0;">
                    <div style="font-weight: 500; margin-bottom: 4px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${escapeHtml(task.title || '无标题')}">
                        ${escapeHtml(task.title || '无标题')}
                    </div>
                    <div style="font-size: 12px; color: var(--text-secondary);">
                        ${escapeHtml(task.authorName || task.author || '未知作者')}
                        ${task.progress !== undefined ? ` · ${task.progress.toFixed(1)}%` : ''}
                        ${task.downloadedMB !== undefined ? ` · ${task.downloadedMB.toFixed(2)} MB` : ''}
                    </div>
                </div>
                <div>
                    <span class="download-status ${statusClass}" style="font-size: 12px;">${statusText}</span>
                    ${task.error ? `<div style="font-size: 11px; color: var(--danger-color); margin-top: 4px;" title="${escapeHtml(task.error)}">${escapeHtml(task.error.length > 30 ? task.error.substring(0, 30) + '...' : task.error)}</div>` : ''}
                </div>
            </div>
        `;
    });
...
// 渲染任务列表（下载队列页面）
function renderQueueBatchTasksList(tasks) {
    const container = document.getElementById('queueBatchTasksList');
    if (!container) return;
...
async function cancelBatchDownload() {
    ...
    try {
        ...
        if (result.success) {
            ...
            // 立即更新一次进度，以便正确显示按钮状态
            try {
                const progressData = await ApiClient.getBatchProgress();
                if (progressData.success) {
                    // 更新统计数据
                    document.getElementById('batchTotal').textContent = progressData.total || 0;
                    document.getElementById('batchDone').textContent = progressData.done || 0;
                    document.getElementById('batchFailed').textContent = progressData.failed || 0;
                    document.getElementById('batchRunning').textContent = progressData.running || 0;

                    // 更新任务列表（如果需要显示任务列表，取消下面的注释并添加对应的HTML容器）
                    // if (progressData.tasks && Array.isArray(progressData.tasks)) {
                    //     renderBatchTasksList(progressData.tasks);
                    // }

                    // 更新按钮状态
                    const isCompleted = progressData.done + progressData.failed === progressData.total && progressData.total > 0;
                    const isCancelled = progressData.running === 0 && progressData.total > 0 && !isCompleted;
                    updateBatchButtons(isCompleted, progressData.failed || 0, isCancelled);
                }
            } catch (e) {
                console.error('Failed to update progress after cancel:', e);
            }
            ...
            // 更新下载队列页面的按钮状态（取消状态）
            updateQueueBatchButtons(false, 0, true);
        } else {
...
// 更新按钮状态（批量下载页面）
function updateBatchButtons(isCompleted, failedCount = 0, isCancelled = false) {
...
// 更新按钮状态（下载队列页面）
function updateQueueBatchButtons(isCompleted, failedCount = 0, isCancelled = false) {
    const cancelBtn = document.getElementById('queueBatchCancelBtn');
    const resumeBtn = document.getElementById('queueBatchResumeBtn');
    const clearBtn = document.getElementById('queueBatchClearBtn');
...
// 继续下载（从pending状态恢复）
async function resumeBatchDownload() {
    try {
        const forceRedownload = document.getElementById('forceRedownload')?.checked || false;
        const result = await ApiClient.resumeBatchDownload(forceRedownload);
        if (result.success) {
            showMessage(`继续下载已启动，待处理任务: ${result.pending || 0} 个`, 'success');
            addBatchLogEntry(`▶️ 继续下载: ${result.pending || 0} 个待处理任务`, 'info');
            ...
            // 显示下载队列页面的进度卡片
            const queueBatchProgressCard = document.getElementById('queueBatchProgressCard');
            if (queueBatchProgressCard) queueBatchProgressCard.style.display = 'block';

            // 立即更新一次进度数据
            await updateBatchProgress();
            await updateQueueBatchProgress();

            // 确保任务列表显示（如果需要显示任务列表，取消下面的注释并添加对应的HTML容器）
            // try {
            //     const progressData = await ApiClient.getBatchProgress();
            //     if (progressData.success && progressData.tasks && Array.isArray(progressData.tasks)) {
            //         renderBatchTasksList(progressData.tasks);
            //         renderQueueBatchTasksList(progressData.tasks);
            //     }
            // } catch (e) {
            //     console.error('Failed to update task list:', e);
            // }

            // 开始进度轮询
            startBatchProgressMonitoring();

            // 如果下载队列页面正在显示，也启动那里的轮询
            if (window.queueBatchProgressInterval) {
                clearInterval(window.queueBatchProgressInterval);
            }
            updateQueueBatchProgress();
            ...
            // 更新按钮状态
            updateBatchButtons(false, 0, false);
            updateQueueBatchButtons(false, 0, false);
        } else {
            showMessage('继续下载失败: ' + (result.error || '未知错误'), 'error');
        }
    } catch (e) {
        showMessage('请求失败: ' + e.message, 'error');
    }
...
// 清除批量下载任务
async function clearBatchTasks() {
    if (!confirm('确定要清除所有批量下载任务吗？此操作不可恢复。')) return;

    try {
        const result = await ApiClient.clearBatchTasks();
        if (result.success) {
            showMessage(`已清除 ${result.cleared || 0} 个任务`, 'success');
            addBatchLogEntry(`🗑️ 已清除所有任务`, 'info');
            ...
            // 隐藏下载队列页面的进度卡片
            const queueBatchProgressCard = document.getElementById('queueBatchProgressCard');
            if (queueBatchProgressCard) queueBatchProgressCard.style.display = 'none';

            // 重置批量下载页面的进度显示
            const el = (id) => document.getElementById(id);
            if (el('batchTotal')) el('batchTotal').textContent = '0';
            if (el('batchDone')) el('batchDone').textContent = '0';
            if (el('batchFailed')) el('batchFailed').textContent = '0';
            if (el('batchRunning')) el('batchRunning').textContent = '0';
            const batchProgressBar = el('batchProgressBar');
            if (batchProgressBar) {
            ...
            // 重置下载队列页面的进度显示
            if (el('queueBatchTotal')) el('queueBatchTotal').textContent = '0';
            if (el('queueBatchDone')) el('queueBatchDone').textContent = '0';
            if (el('queueBatchFailed')) el('queueBatchFailed').textContent = '0';
            if (el('queueBatchRunning')) el('queueBatchRunning').textContent = '0';
            const queueBatchProgressBar = el('queueBatchProgressBar');
            const queueBatchProgressText = el('queueBatchProgressText');
            if (queueBatchProgressBar) queueBatchProgressBar.style.width = '0%';
            if (queueBatchProgressText) queueBatchProgressText.textContent = '0%';

            // 隐藏当前任务信息
            const queueBatchCurrentTask = el('queueBatchCurrentTask');
            if (queueBatchCurrentTask) queueBatchCurrentTask.style.display = 'none';

            // 重置状态文本
            const queueBatchStatusText = el('queueBatchStatusText');
            if (queueBatchStatusText) {
                queueBatchStatusText.textContent = '';
                queueBatchStatusText.style.color = '';
            }

            // 重置按钮状态
            updateBatchButtons(false);
            updateQueueBatchButtons(false);

            // 清空日志
            clearBatchLog();
        } else {
            showMessage('清除失败: ' + (result.error || '未知错误'), 'error');
        }
    } catch (e) {
        showMessage('请求失败: ' + e.message, 'error');
    }
